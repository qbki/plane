...
make_block(std::shared_ptr<entt::registry>& registry)
{
  ...
  layout.add([registry, entity](auto&) {
    ...
    for (auto& child_entity : std::views::reverse(children.value)) {
      auto [child_transform, child_rect_size] =
        registry->get<Transform, RectSize>(child_entity);
      ...
      child_transform.translate({ child_position.x,
                                  ...
                                  child_position.z });
      new_rect_size.width =
        glm::max(new_rect_size.width, child_rect_size.width);
      new_rect_size.height += child_rect_size.height;
    }
    rect_size = new_rect_size;

    RectSize parent_rect_size =
      parent.value.has_value() ? registry->get<RectSize>(parent.value.value())
